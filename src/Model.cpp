...
void Model::loadMesh(unsigned int indMesh) {
    ...
    unsigned int normAccInd = JSON["meshes"][indMesh]["primitives"][0]["attributes"]["NORMAL"];
    unsigned int texAccInd = JSON["meshes"][indMesh]["primitives"][0]["attributes"]["TEXCOORD_0"];
    unsigned int indAccInd = JSON["meshes"][indMesh]["primitives"][0]["indices"];

    std::vector<float> posVec = getFloats(JSON["accessors"][posAccInd]);
    std::vector<glm::vec3> positions = groupFloatsVec3(posVec);
    std::vector<float> normalVec = getFloats(JSON["accessors"][normAccInd]);
    std::vector<glm::vec3> normals = groupFloatsVec3(normalVec);
    std::vector<float> texVec = getFloats(JSON["accessors"][texAccInd]);
    std::vector<glm::vec2> texUVs = groupFloatsVec2(texVec);

    std::vector<PosNormTex> vertices = assembleVertices(positions, normals, texUVs);
...
void Model::traverseNode(unsigned int nextNode, glm::mat4 matrix) {
    ...
    if (node.find("matrix") != node.end()) {
        ...
        if (node.find("mesh") != node.end()) {
            ...
            matrixMeshes.push_back(matNextNode);
...
std::vector<PosNormTex> Model::assembleVertices
(
...
char* Model::getFileContents(const char* filename) {
